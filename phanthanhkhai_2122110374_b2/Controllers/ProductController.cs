...
<<<<<<< HEAD
        ...
        public IActionResult GetProductImage(int id)
        {
            ...
            var imagePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/images", $"product{id}.jpg");
        ...
        public async Task<IActionResult> GetProductsByCategory(int categoryId)
        {
            ...
            var products = await _context.Products
                .Where(p => p.Cat_Id == categoryId) // Lọc theo Cat_Id
                .ToListAsync();

            if (products == null || !products.Any())
            {
                return NotFound(new { message = "Không có sản phẩm nào trong danh mục này." }); // Trả về 404 nếu không có sản phẩm
            }
